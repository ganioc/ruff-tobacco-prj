...
export class MqttApp {
    ...
    public updateReport(running: boolean) {
        if (this.lost) {
            return;
        }
...
//                    stage_remain_min: this.minutes, // in minutes
            ...
            if (running) {
                data.reported.stage = this.appBaking.bakingCurve.indexBakingElement;
                data.reported.stage_remain_min = info.RunningCurveInfo.TempDurationList[stage] * 60 - Math.round(this.appBaking.bakingCurve.getCurrentStageElapsedTime() / 60);
            }
